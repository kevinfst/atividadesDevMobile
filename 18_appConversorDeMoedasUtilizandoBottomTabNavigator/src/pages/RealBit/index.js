...
const BitCoin= () => {
  ...
  const [moedaDestino, setMoedaDestino] = useState('bitcoin');
  const [exibirResultado, setExibirResultado] = useState('');

  const taxaConversao = { bitcoin: 0.000027,
  };
  ...
  const converterMoeda = () => {
    ...
    const resultadoFormatado = 'RESULTADO: ' + valorConvertido.toFixed(8) + ' BTC';
